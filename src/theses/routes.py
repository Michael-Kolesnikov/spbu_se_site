...
from src.models import Courses, Staff, Thesis, Users, Worktype
from src.theses.forms import ThesisFilter
...
def theses_search():
    ...
    return render_template("theses/theses.html", filter=filter, hint=hint)


@bp.route("/fetch_theses")
def fetch_theses():
    ...
    if len(records.items):
        ...
        return render_template("theses/fetch_theses_blank.html")


@bp.route("/post_theses", methods=["GET", "POST"])
def post_theses():
    error_status = 500
    success_status = 0
    thesis_text = None
...
@bp.route("/theses_tmp.html")
def theses_tmp():
    records = Thesis.query.filter_by(temporary=True).filter_by(review_status=10)
    return render_template("theses/theses_tmp.html", theses=records)


@bp.route("/theses_delete_tmp")
def theses_delete_tmp():
    ...
    return redirect(url_for("theses.theses_tmp"))


@bp.route("/theses_add_tmp")
def theses_add_tmp():
    thesis_id = request.args.get("thesis_id", default=1, type=int)
    thesis = Thesis.query.filter_by(id=thesis_id).filter_by(temporary=True).first()
