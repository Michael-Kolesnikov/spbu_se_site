...
from src.practice.student_template import PracticeStudentTemplates
from typing import List
from src.practice.forms import CurrentWorktypeArea, ChooseTopic, UserAddReport
from functools import wraps
from src.notification import add_mail_notification, NotificationTemplates
...
def get_filename(
    ...
    filename = (
        author_en
        + "_"
        + current_app.config["type_id_string"][current_thesis.worktype_id - 1]
    )
...
bp.add_url_rule("/practice", methods=["GET", "POST"], view_func=practice_index)
bp.add_url_rule("/practice/", methods=["GET", "POST"], view_func=practice_index)


@bp.route("/practice/guide/", methods=["GET"])
@login_required
def practice_guide():
    return render_template(
...
@bp.route("/practice/new/", methods=["GET", "POST"])
@login_required
def practice_new_thesis():
    if request.method == "POST":
        ...
        if current_worktype_id == 0:
            ...
            db.session.commit()
            return redirect(
                url_for("practice.practice_choosing_topic", id=new_thesis.id)
            )
...
@bp.route("/practice/data_for_practice/", methods=["GET", "POST"])
@login_required
...
@bp.route("/practice/choosing_topic/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
def practice_choosing_topic(current_thesis):
    if request.method == "POST":
        if "save_topic_button" in request.form:
            ...
            elif len(topic) <= current_app.config["MIN_LENGTH_OF_TOPIC"]:
...
@bp.route("/practice/edit_theme/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
...
@bp.route("/practice/goals_tasks/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
def practice_goals_tasks(current_thesis):
    if request.method == "POST":
        if "submit_goal_button" in request.form or "edit_goal_button" in request.form:
            ...
            if len(goal) <= current_app.config["MIN_LENGTH_OF_GOAL"]:
                flash(
            ...
            if len(task) <= current_app.config["MIN_LENGTH_OF_TASK"]:
            ...
            if len(new_task) <= current_app.config["MIN_LENGTH_OF_TASK"]:
                flash("Опишите задачу подробнее!", category="error")
                return redirect(url_for("practice_goals_tasks", id=current_thesis.id))
...
@bp.route("/practice/add_new_report/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
def practice_add_new_report(current_thesis):
    if request.method == "POST":
        ...
        if not was_done:
        ...
        elif len(was_done) <= current_app.config["MIN_LENGTH_OF_FIELD_WAS_DONE"]:
            flash(
            ...
            len(planned_to_do)
            <= current_app.config["MIN_LENGTH_OF_FIELD_PLANNED_TO_DO"]
        ):
            flash(
...
@bp.route("/practice/workflow/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
...
@bp.route("/practice/preparation_for_defense/", methods=["GET", "POST"])
@login_required
@current_thesis_exists_or_redirect
        ...
        if "submit_text_button" in request.form:
            ...
            if text_file is not None and text_file.filename != "":
                full_filename, filename = get_filename(
                    current_thesis,
                    current_app.config["TEXT_UPLOAD_FOLDER"],
                    current_app.config["TEXT"].value,
                )
            ...
            if supervisor_review:
                full_filename, filename = get_filename(
                    current_thesis,
                    current_app.config["REVIEW_UPLOAD_FOLDER"],
                    current_app.config["SUPERVISOR_REVIEW.value"],
                )
            ...
            if reviewer_review:
                full_filename, filename = get_filename(
                    current_thesis,
                    current_app.config["REVIEW_UPLOAD_FOLDER"],
                    current_app.config["REVIEWER_REVIEW"].value,
                )
            ...
            if presentation_file is not None and presentation_file.filename != "":
                full_filename, filename = get_filename(
                    current_thesis,
                    current_app.config["PRESENTATION_UPLOAD_FOLDER"],
                    current_app.config["PRESENTATION"].value,
                )
...
@bp.route("/practice/defense/", methods=["GET"])
@login_required
@current_thesis_exists_or_redirect
