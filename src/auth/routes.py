...
@bp.route("/register_basic.html", methods=["GET", "POST"])
def register_basic():
    if request.method == "POST":
...
@bp.route("/password_recovery.html", methods=["GET", "POST"])
def password_recovery():
    return render_template("password_recovery.html")


@bp.route("/profile.html", methods=["GET", "POST"])
@login_required
def user_profile():
...
@bp.route("/google_callback", methods=["GET"])
def google_callback():
    state = session["state"]
